...
//			result[2] = 0x00;
			byte[] time = UtilTools.nowTimeToBytes();
		...
		case STATE_DEVICE_SET_ALARM:
			...
			byte [] alarmToBytes1 = UtilTools.alarmToBytes1();
			byte [] alarmToBytes2 = UtilTools.alarmToBytes2();
			byte [] alarmToBytes3 = UtilTools.alarmToBytes3();
		...
		case STATE_DEVICE_WALK:
			Log.i(TAG, "计步目标设定 ");
			result[0] = getDataHeader(3);
			byte[] b = UtilTools.intToByteArray(UtilTools.sports);
			System.arraycopy(b, 0, result, 4, b.length);
			break;
		case USER_SET:
			
			result[0] = getDataHeader(3);
			byte[] user = UtilTools.userToByte();
...
//			start = System.currentTimeMillis();
		...
		case LONG_SIT:
			...
			byte[] lgSit = UtilTools.longSitByte();
