...
//        void onStopTrackingTouch();
    ...
    private void initViewAttrs(AttributeSet attrs){
        ...
        int progressBackgroundColor = localTypedArray.getColor(R.styleable.CircleSeekBar_progress_background, Color.parseColor("#44000000"));//背景颜色黑色
        int progressFrontColor = localTypedArray.getColor(R.styleable.CircleSeekBar_progress_front, Color.parseColor("#ff31a1d3"));//背景颜色红色
        ...
        int progressTextColor = localTypedArray.getColor(R.styleable.CircleSeekBar_progress_text_color, Color.parseColor("#ff323232"));//字体颜色
        mProgressTextSize = (int) localTypedArray.getDimension(R.styleable.CircleSeekBar_progress_text_size, 40);
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 起始位置，三点钟方向
        setThumbPosition(Math.toRadians(-90));
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(this.mArcRectF, -90, mSeekBarDegree, false, mSeekbarProgressPaint);
    ...
    private void drawProgressText(Canvas canvas) {
        if (true == mIsShowProgressText){
            String   TextTemp = this.getMin() + ":" + this.getSec();
            float textWidth = mProgressTextPaint.measureText("" + mCurrentProgress);
            if(mCurrentProgress<10){
                canvas.drawText(TextTemp, mSeekBarCenterX - textWidth *2, mSeekBarCenterY
                        + mProgressTextSize / 2, mProgressTextPaint);
            }else if(mCurrentProgress>99){
                canvas.drawText(TextTemp, mSeekBarCenterX  - (textWidth/3)*2 , mSeekBarCenterY
                        + mProgressTextSize / 2, mProgressTextPaint);
            }else {
                canvas.drawText(TextTemp, mSeekBarCenterX - textWidth  , mSeekBarCenterY
                        + mProgressTextSize / 2, mProgressTextPaint);
            }
            Log.i(TAG,"mSeekBarCenterXextWidth:"+(mSeekBarCenterX - textWidth));
            Log.i(TAG,"textWidth:"+ textWidth +",mProgressTextPaint:"+ mProgressTextPaint);
        }
    }
    public CharSequence getMin() {
        int min = mCurrentProgress / 60;
        return min < 10 ? "0" + min : String.valueOf(min);
    }

    public CharSequence getSec() {
        int sec = mCurrentProgress % 60;
...
//                    mOnSeekBarChangeListener.onStopTrackingTouch();
    ...
    private void seekTo(float eventX, float eventY, boolean isUp) {
        if (true == isPointOnThumb(eventX, eventY) && false == isUp) {
            ...
            double radian = Math.atan2(eventY - mSeekBarCenterY, eventX - mSeekBarCenterX) +0.5*Math.PI;
            /*
             * 由于atan2返回的值为[-pi,pi]
            ...
            setThumbPosition(radian -0.5*Math.PI);
