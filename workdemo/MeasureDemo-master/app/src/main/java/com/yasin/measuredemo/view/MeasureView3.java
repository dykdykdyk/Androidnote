...
public class MeasureView3 extends View {
    //弧形开始的角度
    ...
    //里面半圆的半径
    private int innerRadius = dp2px(60);
    //最内层的padding
    ...
    //刻度线的高度
    private int outerLineHeight = dp2px(2);
    //文字的size
    ...
    //当前进度
    private float currProgress = 0f;
    private float currProgress2 = 0f;
    //起始值
    private float start = 0;
    //最终值
    private float end = 150;

    public MeasureView3(Context context) {
        this(context, null);
    }

    public MeasureView3(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MeasureView3(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        //画出最里面的弧线
        drawCenterStaticLine(canvas);//外部的线
                drawText(canvas);//字体
        drawProgress(canvas);//进度条
    }

    private void drawCenterStaticLine(Canvas canvas) {
        //最外面的进度条的默认黑色
        RectF rectF1 = new RectF();
        int width = getWidth();
        rectF1.set(innerLineWidth+progressLineW, innerLineWidth+progressLineW, width - innerLineWidth-progressLineW, getHeight() * 2 - innerLineWidth);
        linePaint.setStrokeWidth(progressLineW);
        linePaint.setColor(PROGRESS_COLOR);
        ...
        linePaint.setColor(Color.WHITE);
        rectF2.set(innerLineWidth + outerPadding + outerPadding / 2+progressLineW, innerLineWidth + outerPadding + outerPadding / 2+progressLineW,
                width - (innerLineWidth + outerPadding + outerPadding / 2+progressLineW), getHeight() * 2 - ((innerLineWidth + outerPadding + outerPadding / 2)));
        canvas.drawArc(rectF2, startAngle, sweepAngle, false, linePaint);
        //最里面的进度条默认黑色
        progressPaint.setColor(PROGRESS_COLOR);
        RectF rectF3 = new RectF();
        rectF3.set(innerLineWidth + outerPadding + outerPadding * 2+progressLineW, innerLineWidth + outerPadding + outerPadding * 2+progressLineW,
                width - (innerLineWidth + outerPadding + outerPadding * 2+progressLineW), getHeight() * 2 - ((innerLineWidth + outerPadding + outerPadding / 2)));
        canvas.drawArc(rectF3, startAngle, sweepAngle, false, progressPaint);
    }
    ...
    private void drawProgress(Canvas canvas) {
        ...
        rectF3.set(innerLineWidth + outerPadding + outerPadding * 2+progressLineW, innerLineWidth + outerPadding + outerPadding * 2+progressLineW,
                width - (innerLineWidth + outerPadding + outerPadding * 2+progressLineW), getHeight() * 2 - ((innerLineWidth + outerPadding + outerPadding / 2)));
        canvas.drawArc(rectF3, startAngle, sweepAngle* currProgress, false, progressPaint);

        //最外面的进度条的添加白色
        RectF rectF1 = new RectF();
        rectF1.set(innerLineWidth+progressLineW, innerLineWidth+progressLineW, width - innerLineWidth-progressLineW, getHeight() * 2 - innerLineWidth);
        progressPaint.setColor(Color.WHITE);
        canvas.drawArc(rectF1, startAngle, sweepAngle* currProgress2, false, progressPaint);
    ...
    private void drawText(Canvas canvas) {
        ...
        String currText = String.valueOf(((int) (start + (end - start) * currProgress2))+"/"+((int) (start + (end - start) * currProgress)));
        //因为数字字体大而单位数字小
        textPaint.setTextSize(textSize);
        //测量数字文字对应的长度
    ...
    public void setProgress(float progress2) {
        if (currProgress2 == progress2 ) {
            return;
        }
        Log.i("TAG","progress2: "+progress2);
        this.currProgress2 = progress2;
        if (Looper.myLooper() == Looper.getMainLooper()) {
            invalidate();
        } else {
            postInvalidate();
