...
public class LineChartActivity_Custom extends Activity  {
...
int i =0;
    ...
    public void ReFreshLineChart(String x, final int y,final LineChart chart,
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if(DataList.size()>1){//Y轴数据
                    vals.add(new Entry( f-(f-Float.parseFloat(XDataList.get(XDataList.size()-2)))/2,DataList.get(DataList.size()-2)));//X轴第一个数据
                    vals.add(new Entry( f-(f-Float.parseFloat(XDataList.get(XDataList.size()-2)))/2,y));//X轴第一个数据
                }
...
//                }
                i++;
                ReFreshLineChart(i*2+"",3+i*2,lineChart,values1);
