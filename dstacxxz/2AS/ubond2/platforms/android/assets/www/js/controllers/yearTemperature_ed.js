angular.module('ufind1.controllers')
    .controller('yearTemperatureCtrl', function ($scope, $state, $stateParams, $timeout, $ionicPopup,$translate,$ionicLoading, dialogsManager, connectedService,temperatureService,temperatureTimeService,decService, bleService) {
        var myChart = echarts.init(document.getElementById('yearTemperature'), 'shine');
...
//        console.log("temperature_time--->" + temperature_time);
        ...
        var updateData = function () {
            if (temperature_time.length <= 96 * 365) {
                $scope.x_time = temperature_time;
                $scope.y_heart = temperature;
            } else {
                $scope.y_heart = temperature.slice(temperature.length - 96 * 365, temperature.length);
                $scope.x_time = temperature_time.slice(temperature_time.length - 96 * 365, temperature_time.length);
            }
        }
        updateData();
...
//            console.log(" $scope.oneday_temp "+ JSON.stringify( $scope.oneday_temp))
...
//                                console.log("更新tempTimeArray.slice(ii+1)----->" + JSON.stringify(tempTimeArray.slice(ii)));
...
//            $state.go("mothTemperatureHistory");
//        }
