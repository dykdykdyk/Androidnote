(function () {
    'use strict';
    angular.module('heightpicker_alarm', [])
        .directive('itemPickerAlarm', ['$timeout', '$ionicScrollDelegate',
            function ($timeout, $ionicScrollDelegate) {
                return {
                    templateUrl: 'lib/timepicer/itemPicker_alarm.html',
                    restrict: 'AE',
                    replace: true,
                    scope: {option: '='},
                    link: function (scope, elements, attributes) {
                        ...
                        scope.height = scope.option.height ;

                        console.log("scope.option.monthSelected"+ scope.option.monthSelected )
                        ...
                        function initMoth(options) {
                            ...
                            prependLi(scope.monthList, 2, "b")
                            for (var i = 0; i < 60; i++) {
                                data = prependZero(options.monthList[i], 10) ;
                                text =  prependZero(options.monthList[i], 10) ;
                        ...
                        function initYear(options) {
                            var text, data, top, item, defaultItem, defaultIndex;
                            prependLi(scope.itemList, 2, "b")
                            for (var i = 0; i <24; i++) {
                                text = prependZero(scope.option.itemList[i], 10);
                                data = prependZero(scope.option.itemList[i], 10);
                                top = 40 + scope.itemList[scope.itemList.length - 1].top;
                                item = createDateTimeLi(10, top, data, i == scope.option.itemSelected, text);
                                scope.itemList.push(item);
                            }
                            prependLi(scope.itemList, 2, "e")
                        }
                        $timeout(function () {
                        ...
                        function updateSelect(index, type) {
                            ...
                            switch (type) {
                                case "year":
                                    //强制
                                    $timeout(function () {
                                        scope.itemList[scope.option.itemSelected + 2].selected = false;
...
//                                        scope.selectDateTime.year.index = index;
                                ...
                                case "month":
                                    //强制
                                    $timeout(function () {
                                        scope.monthList[scope.option.monthSelected + 2].selected = false;
...
//                                        scope.selectDateTime.month.index = index;
                        ...
                        function updateSelect_item(index) {
                            $timeout(function () {
                                scope.itemList[scope.option.itemSelected + 2].selected = false;
                                scope.itemList[index + 2].selected = true;
                                scope.option.itemSelected = index;
                            });
                        }
                        function updateSelect_month(index) {
                            $timeout(function () {
                                scope.monthList[scope.option.monthSelected + 2].selected = false;
                                scope.monthList[index + 2].selected = true;
                                scope.option.monthSelected = index;
                            });
                        }

                        scope.selectEvent1 = function (index) {
                            if (index > 1 && index <= scope.itemList.length - 2) {

                                updateSelect_item(index - 2);
                                scrollToElm(scope.itemScroll, scope.itemList[index - 2]);
//                                scope.option.itemSelected = index - 3;
                            }
                           };
                        scope.selectEvent2 = function (index) {
                            if (index > 1 && index <= scope.monthList.length - 2) {

                                updateSelect_month(index - 2);
                                scrollToElm(scope.monthScroll, scope.monthList[index - 2]);
//                                scope.option.itemSelected = index - 3;
                            }
                           };
