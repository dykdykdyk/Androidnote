...
public class SecurityMessageHandler extends MessageHandler{
	...
	public SecurityMessageHandler(BLEDevice peripheral) {
		super(peripheral);
	}
...
//			Log.i(TAG, "设备响应登录请求");
			mBLEDevice.sendLogin(data[4]);
			break;
			
		case STATE_PHONE_BOUND:
//			Log.i(TAG, "设备响应绑定请求"); 
            mBLEDevice.sendBound(data[4]);
			break;
...
//			mPeripheral.login(data[4]);
		...
		case STATE_DEVICE_FUNCTION:
			...
			L.i(TAG, "设备特性..."+ "血氧:"+oxygen+ " 血压:" +blood+" 温度:"+temperature +" 心率:"+heart+" 睡眠:"+sleep+" 计步:"+step);
			mBLEDevice.sendFunction(oxygen, blood, temperature, heart, sleep, step);
			break;
		}
	}
