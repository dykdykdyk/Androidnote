...
public class ChartsFragment extends Fragment implements Update {
	private View rootView = null;// 缓存Fragment view

	private BLEInfoActivity mBleInfoActivity;
	...
	public ChartsFragment(Peripheral periheral) {
		this.mPeripheral = periheral;
		key = periheral.getAddres();
		mPeripheral.setUpdateChatrs(this);
	}
...
//			line1.setStrokeWidth(2);// 线条的粗细，默认是3
...
//			line2.setHasPoints(true);// 是否显示圆点 如果为false 则没有原点只有点显示
	...
	private void setTemp(int temp) {
		...
		Date date = new Date();
		String dateStr = format.format(date);
		mTempValues.add(new PointValue(xx, a));
		mAxisXValues.add(new AxisValue(xx).setLabel(dateStr));
		setPoint();
	}
	...
	@Override
	public void sendString(String key, String s) {
	}

	@Override
	public void sendSports(String address, final int s) {
		if (key.equals(address)) {
			if (mBleInfoActivity != null) {
				mBleInfoActivity.runOnUiThread(new Runnable() {
					public void run() {
						String x = s + "";
						step_tv_view.setText(x);
					}
				});
			}
		}
	...
	@Override
	public void sendHeart(String address, final int s) {
		if (key.equals(address)) {
			if (mBleInfoActivity != null) {
				mBleInfoActivity.runOnUiThread(new Runnable() {
					public void run() {
	...
	@Override
	public void sendTemp(String address, final int s) {
		if (key.equals(address)) {
			if (mBleInfoActivity != null) {
				mBleInfoActivity.runOnUiThread(new Runnable() {
					public void run() {
	...
	@Override
	public void sendPa(String address, final float pa, final float altitude,
			final float degree) {
		if (key.equals(address)) {
			if (mBleInfoActivity != null) {
				mBleInfoActivity.runOnUiThread(new Runnable() {
					public void run() {
						// setTemp(s);
						// String t = s / 10f + "";
						setPa(pa,altitude,degree);
						al_tv_view.setText((altitude + "m"));
						at_tv_view.setText((pa + "Kpa"));
						am_tv_view.setText((degree + "°"));
