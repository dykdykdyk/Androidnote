	...
	public void handleMessage(byte[] data) {
		...
		case STATE_DEVICE_BACK_ALARM:
			for(int i = 0;i<3;i++){
				...
				int opend = (a5 & 0xff)>> 7; 
//				String a =  "返回闹钟列表--->"+"年："+year1+" 月："+month1+" 日："+day1
//						+" 时："+hour1+" 分："+min1+" id:"+alarmID1+" 重复日:"+dayFlags1;
			
				String a ="获取返回闹钟列表0x03--->"+""+year1+"-"+month1+"-"+day1
...
//						+" 时："+hour1+" 分："+min1+" id:"+alarmID1+" 重复日:"+dayFlags1;
				...
				String a ="返回闹钟列表0x02--->"+""+year1+"-"+month1+"-"+day1
						+"  "+hour1+" ："+min1+" /id:"+alarmID1+" 重复日:"+dayFlags1 + "开关："+opend;
				LogUpdate.i(TAG, key,a, 0, 1);
			}
		    break;
...
//			Log.i(TAG, "返回时间设置");	
			int y = data[4]>>2;
			LogUpdate.i(TAG,key, "返回时间设置 "+ data[4],0, 1);
//			Log.i(TAG, "返回时间设置--- "+"年："+y);	
			break;
		case USER_SET:
			LogUpdate.i(TAG,key, "返回用户设置 "+ data[4],0, 1);
			break;	
		case STATE_DEVICE_WALK:
			LogUpdate.i(TAG,key, "返回计步目标设定 "+ data[4],0, 1);
			
			break;
		case STATE_DEVICE_LOST:
			LogUpdate.i(TAG,key, "返回防丢设置 "+ data[4],0, 1);
//			Log.i(TAG, "返回防丢设置"+ data[4]);
			break;
		case LONG_SIT:
			LogUpdate.i(TAG,key, "返回久坐提醒设置  "+ data[4],0, 1);
//			Log.i(TAG, "返回久坐提醒设置 "+data[4]);
			break;
		case FactorySettings:
			LogUpdate.i(TAG,key, "返回出厂设置 "+ data[4],0, 1);
//			Log.i(TAG, "返回出厂设置 ");
			break;
...
//			Log.i(TAG, "获取设备闹钟列表请求 ");
		...
		case STATE_DEVICE_WALK:
//			Log.i(TAG, "计步目标设定 ");
			LogUpdate.i(TAG, key,"计步目标设定 ", 0, 1);
			result[0] = getDataHeader(3);
			byte[] b = Util.intToByteArray(5000);
			System.arraycopy(b, 0, result, 4, b.length);
			break;
		case USER_SET:
//			Log.i(TAG, "用户信息设置命令 ");
			LogUpdate.i(TAG, key,"用户信息设置命令", 0, 1);
			result[0] = getDataHeader(3);
...
//			Log.i(TAG, "防丢设置---->好像没反应");
			LogUpdate.i(TAG, key,"防丢设置", 0, 1);
			result[0] = getDataHeader(0);
			result[4] = 0x02; //0:不报警；1:普通报警；02:紧急报警 不能写死
			break;
		case LONG_SIT:
//			Log.i(TAG, "久坐提醒设置 ");
			LogUpdate.i(TAG, key,"久坐提醒设置 ", 0, 1);
...
//			break;	
		case FactorySettings:
//			Log.i(TAG, "恢复出厂设置 ");
			LogUpdate.i(TAG, key,"恢复出厂设置 ", 0, 1);
			result[0] = getDataHeader(0);
			result[4] = 0x00;
			break;	
